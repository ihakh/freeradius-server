...
#define TMPL_RETURN(_vpt) return -((_vpt) == lhs ? fr_sbuff_used(m_lhs) : fr_sbuff_used(m_rhs))
	 ...
	 */
	if (TMPL_OF_TYPE_A_NOT_A(attr, vpt)) {
		if ((tmpl_da(attr)->type == FR_TYPE_STRING) &&
		    (op != T_OP_CMP_TRUE) &&
		    (op != T_OP_CMP_FALSE) &&
			     ...
			     tmpl_rules_t const *rules)
{
	...
	if (fr_sbuff_next_if_char(&our_in, '(')) {
		...
		slen = cond_tokenize(c, &c->data.child, cs, &our_in, brace + 1, rules);
		if (slen <= 0) {
			fr_sbuff_advance(&our_in, slen * -1);
			goto error;
		}
	...
	slen = cond_tokenize_operand(c, &lhs, &m_lhs, &our_in, rules);
	if (!lhs) {
		fr_sbuff_advance(&our_in, slen * -1);
		goto error;
	}
...

	{
		...
		slen = cond_tokenize_operand(c, &rhs, &m_rhs, &our_in, rules);
		if (!rhs) {
			fr_sbuff_advance(&our_in, slen * -1);
			goto error;
		}
...
closing_brace:
	 ...
	 */
	slen = cond_tokenize(c, &c->next, cs, &our_in, brace, rules);
	if (slen <= 0) {
		fr_sbuff_advance(&our_in, slen * -1);
		goto error;
	}
